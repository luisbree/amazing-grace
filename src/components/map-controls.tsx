...
import { 
  ...
  Square, PenLine, Dot, Ban, Eraser, Save, ListFilter, Download, MapPin
} from 'lucide-react';
import {
  Accordion,
  AccordionContent,
...
const MapControls: React.FC<MapControlsProps> = ({ 
  ...
  React.useEffect(() => {
    if (renderConfig.import && (selectedFile || selectedMultipleFiles)) {
      handleFileUpload();
    }
  }, [selectedFile, selectedMultipleFiles, handleFileUpload, renderConfig.import]);
            ...
            <AccordionContent className="p-0 pt-0 border-t border-white/10 bg-black/10 rounded-b-md">
              {layers.length === 0 ? (
                <div className="text-center py-6 px-3">
                  ...
                  {renderConfig.import && <p className="text-xs text-gray-400/70">Use el importador.</p>}
                </div>
              ) : (
